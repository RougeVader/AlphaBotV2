...
const formatValueWithUnit = (val: number, label: string): string => {
  if (val == null) return "0";
  const lbl = label.toLowerCase();
  if (lbl.includes('percentage') || lbl.includes('pct') || lbl.includes('rate')) {
    return `${val.toLocaleString()}%`;
  }
  if (lbl.includes('delay') || lbl.includes('days')) {
    return `${val.toLocaleString()} Days`;
  }
  if (lbl.includes('capacity') || lbl.includes('mw')) {
    return `${val.toLocaleString()} MW`;
  }
  if (lbl.includes('headcount') || lbl.includes('count') || lbl.includes('people') || lbl.includes('employees')) {
    return val.toLocaleString();
  }
  if (lbl.includes('revenue') || lbl.includes('profit') || lbl.includes('expenses') || lbl.includes('cost') || lbl.includes('budget') || lbl.includes('spend') || lbl.includes('salary') || lbl.includes('liability') || lbl.includes('value')) {
...
export function DataChart({ results, unit, darkMode }: DataChartProps) {
  ...
  let labelKey = keys.find(key => key.toLowerCase() === 'label');
  if (!labelKey) {
    labelKey = keys.find(key => typeof firstRow[key] === 'string' && key !== 'comparison_group');
  }
  if (!labelKey) {
    labelKey = keys.find(key => typeof firstRow[key] === 'string');
  }
  
  const numericKeys = keys.filter(key => typeof firstRow[key] === 'number' && key !== labelKey);

  // ── Single scalar value ──────────────────────────────────────────────────
  if (results.length === 1 && numericKeys.length === 1) {
    const value = firstRow[numericKeys[0]];
    const formattedVal = formatValueWithUnit(value, numericKeys[0]);
    const isFormatted = formattedVal !== value.toLocaleString();
    ...
    return (
      <div className="flex flex-col items-center justify-center py-8">
        ...
        {!isFormatted && unit && unit !== 'RawData' && (
          <div className="text-sm text-gray-500 dark:text-slate-400">{unit}</div>
        )}
        <div className="text-xs text-gray-400 dark:text-slate-50 mt-1">{metricName}</div>
      </div>
    );
  }

  const isTimeSeries = results.length > 1 && labelKey != null && (
    labelKey.toLowerCase().includes('date') ||
    labelKey.toLowerCase().includes('year') ||
    labelKey.toLowerCase().includes('label') && results.every(r => /^\d{4}(-\d{2})?$/.test(String(r[labelKey])))
  );
  ...
  const scales = {
    ...
    y: {
      ...
      ticks: {
        color: textColor,
        callback: (value: any) => {
            if (unit === 'RawData') return value.toLocaleString();
            const isCurrency = unit === 'USD' || unit === '₹ Cr';
            const prefix = isCurrency ? '₹' : '';
            const suffix = isCurrency ? ' Cr' : (unit === '%' ? '%' : (unit ? ` ${unit}` : ''));
        ...
        tooltip: {
          callbacks: {
            label: (c: any) => {
              ...
              return `${c.label}: ${formatValueWithUnit(val, c.label)}`;
            }
          }
        },
      },
  ...
  if (displayResults.length >= 1 && finalLabelKey) {
    const hasComparisonGroup = displayResults.some(r => 'comparison_group' in r && r.comparison_group != null);
    
    let labels: string[];
    let datasets: any[];
    
    if (hasComparisonGroup) {
      const uniqueLabels = Array.from(new Set(displayResults.map(r => String(r[finalLabelKey]).trim()))).sort();
      labels = uniqueLabels.map(lbl => formatLabel(lbl));
      
      const uniqueGroups = Array.from(new Set(displayResults.map(r => String(r.comparison_group).trim()))).sort();
      
      datasets = [];
      let colorIdx = 0;
      uniqueGroups.forEach((group) => {
        finalNumericKeys.forEach((metric) => {
          const color = borderColors[colorIdx % borderColors.length];
          colorIdx++;
          ...
          const data = uniqueLabels.map(labelVal => {
            const row = displayResults.find(r => 
              String(r[finalLabelKey]).trim() === labelVal && 
              String(r.comparison_group).trim() === group
            );
            return row ? row[metric] : 0;
          });
          
          const seriesLabel = finalNumericKeys.length > 1
            ? `${formatLabel(group)} - ${metric.replace(/_/g, ' ').toUpperCase()}`
            : formatLabel(group);
      ...
      labels = displayResults.map(r => formatLabel(r[finalLabelKey]));
      datasets = finalNumericKeys.map((metric, idx) => {
        ...
        tooltip: {
          callbacks: {
            label: (c: any) => {
              ...
              const label = c.dataset.label || c.label;
              return `${label}: ${formatValueWithUnit(val, label)}`;
            },
          },
        },
      },
